...
项目当前处于 **C3.1 NASA C-MAPSS 最小接入、schema validation 与 CLI report 已实现，准备执行默认 preflight 和 review 决策** 阶段。
...
当前 C3.1 的定位不是“下载 NASA C-MAPSS 数据并训练模型”，而是在默认离线边界下把 source/license preflight、schema validation、canonical observation mapping、RUL 标签语义、split/leakage 风险和 C3.2 Go / No-Go 写成可复核报告。下一步是执行并 review C3.1 default preflight，再做 source/license/local raw opt-in 决策。
...
| 日期 | 当日完成内容 |
| --- | --- |
| 2026-06-10 | 完成 C3.1 NASA C-MAPSS 最小接入实现与文档回归：新增默认离线 config、loader、NASA PCoE #6 经典文件 contract、raw parser、canonical mapping、RUL target metadata、split/leakage guard、Markdown report、CLI `experiment c-stage-c31`、README / details 入口和回归测试；默认仍保持不下载公开数据、不读取本机 raw files、不写 processed data、不运行模型训练。 |
...
下一步主线是执行并 review C3.1 default preflight，而不是直接下载公开数据、整理多个公开数据集、跑跨数据模型评测或进入 B 阶段自研训练。整体顺序必须是：先用默认离线配置生成 C3.1 报告并确认 blocked / No-Go 是否符合 source/license 当前状态；再做 source/license review；只有 source、license 和 training-use 边界明确后，才允许本机 ignored raw mapping 或后续 C3.2。

具体计划如下：

1. 运行 C3.1 default preflight：使用 `configs/c_stage_c31_cmapss_minimal_ingestion.yaml` 生成 `reports/c_stage_c31_cmapss_minimal_ingestion.md`，确认默认 `allow_network: false`、`allow_download: false`、`allow_local_raw_data: false`、`allow_write_processed: false` 下报告仍阻断在 source/license review 边界。
2. Review source/license 结论：确认 NASA PCoE #6 official source、下载入口、引用要求、redistribution 边界和 research training/evaluation use 状态；若任一项不能明确，继续保持 blocked，不进入本机 raw mapping。
3. 决策 local raw opt-in：只有 source/license/training-use 明确后，才允许在 ignored 本机目录放置 C-MAPSS raw files，并通过显式配置打开 `allow_local_raw_data: true`；仍不提交 raw、zip、parquet、报告产物或 cache。
4. 执行本机 raw mapping review：在 opt-in 条件满足时，复核 parser、schema validation、canonical observation mapping、RUL target metadata、split policy 和 leakage guard 的报告输出；发现 schema 或标签语义不匹配时先修正 C3.1，不推进 C3.2。
5. 决策 C3.2：只有完整经典 C-MAPSS schema validation 通过、source/license/training-use 明确、split/leakage guard 无阻断后，才允许设计 C3.2 open model cross-dataset evaluation；C3.2 前仍不下载公开数据到仓库、不提交数据、不训练模型。
