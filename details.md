...
项目当前处于 **C3.1 NASA C-MAPSS local raw mapping review 已通过，下一步进入 C3.2 open model cross-dataset evaluation 设计** 阶段。
...
当前 C3.1 的定位不是“下载 NASA C-MAPSS 数据并训练模型”，而是在默认离线边界下把 source/license review、schema validation、canonical observation mapping、RUL 标签语义、split/leakage 风险和 C3.2 Go / No-Go 写成可复核报告。2026-06-10 source review 已确认 NASA PCoE #6 source 和 S3 download target 身份；license evidence update 进一步记录 Zenodo CC BY 4.0 证据，将 license / redistribution / research training-evaluation use 从 unresolved 推进到 research-only planning eligible。2026-06-11 explicit local raw mapping review 已验证完整经典 C-MAPSS schema、RUL metadata 和 split/leakage guard，状态为 `schema_validated_ready_for_c32`，readiness detail 为 `full_classic_cmapss_validated`。C3.2 可以进入设计，但默认 local raw、download、processed write 和模型训练仍保持 disabled。
...
下一步主线是设计 C3.2 open model cross-dataset evaluation；不是继续停留在 C3.1 授权或 raw mapping，也不是直接训练自研基础模型或扩大到多个未审计公开数据集。C3.1 已经完成 source/license preflight、explicit local raw mapping review、完整 schema validation、RUL metadata 和 split/leakage guard；C3.2 现在应该先设计评估 contract，再落地最小可运行评估。

具体计划如下：

1. 设计 C3.2 open model cross-dataset evaluation：限定为 C-MAPSS 加现有 FU13-like / FU13 证据资产的跨数据评估设计，先定义任务、split、metric、模型候选、报告字段和 Go / No-Go。
2. 保持 C3.2 最小可运行：优先 baseline 与已存在开源模型 adapter contract，不把多个新公开数据集、复杂训练流水线或自研模型训练塞进第一轮。
