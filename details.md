...
项目当前处于 **C3.1 NASA C-MAPSS license evidence 已补齐，下一步设计 explicit local raw mapping review，C3.2 继续 blocked** 阶段。
...
当前 C3.1 的定位不是“下载 NASA C-MAPSS 数据并训练模型”，而是在默认离线边界下把 source/license review、schema validation、canonical observation mapping、RUL 标签语义、split/leakage 风险和 C3.2 Go / No-Go 写成可复核报告。2026-06-10 source review 已确认 NASA PCoE #6 source 和 S3 download target 身份；license evidence update 进一步记录 Zenodo CC BY 4.0 证据，将 license / redistribution / research training-evaluation use 从 unresolved 推进到 research-only planning eligible。默认 local raw 仍 disabled，C3.2 继续 blocked，直到 explicit local raw mapping review 验证完整 schema、RUL metadata 和 split/leakage guard。
...
下一步主线是设计 C3.1 explicit local raw mapping review；不是直接下载公开数据、整理多个公开数据集、跑跨数据模型评测或进入 B 阶段自研训练。C3.1 默认 preflight 已完成并补齐 license evidence，默认 blocked / No-Go 现在来自 local raw mapping 尚未执行；只有 explicit opt-in 的本机 ignored raw mapping review 验证完整 schema、RUL metadata 和 split/leakage guard 后，才允许设计 C3.2。

具体计划如下：

1. 设计 C3.1 explicit local raw mapping review：使用 ignored 本机 raw 目录和显式配置打开 `allow_local_raw_data: true`，仍不提交 raw、zip、parquet、报告产物或 cache。
2. 执行本机 raw mapping review：复核 parser、schema validation、canonical observation mapping、RUL target metadata、split policy 和 leakage guard 的报告输出；发现 schema 或标签语义不匹配时先修正 C3.1，不推进 C3.2。
3. 决策 C3.2：只有完整经典 C-MAPSS schema validation 通过、RUL metadata 明确、split/leakage guard 无阻断后，才允许设计 C3.2 open model cross-dataset evaluation；C3.2 前仍不下载公开数据到仓库、不提交数据、不训练模型。
