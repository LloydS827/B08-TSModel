...
项目已经完成到 **C2.1 开源模型真实执行评测入口**：当前具备统一入口、六模型登记、adapter contract 雏形、task matrix、audit、task attempt 和结构化失败报告。TTM 继续作为真实运行 anchor，Chronos / Chronos-Bolt、TimesFM、Moirai / Uni2TS、MOMENT、UniTS 已进入统一 executable adapter 尝试框架。

下一步主线应进入 **C2.2 open model executable evaluation upgrade**：在 FU13 真实 processed parquet 优先、可控 cache、显式 opt-in 联网/下载边界下，把 C2.1 从“六模型结构化尝试”推进为“升级版开源模型真实执行与 frontier watchlist 审计”。
...
C2.1 已经建立真实执行入口和结构化失败报告，但还没有完成 C2.2 意义上的升级版真实对比。下一步需要把 Chronos-2、TimesFM 2.5、Moirai 2.0 等最新版本纳入真实尝试，并把 Time-MoE、Sundial、Timer-S1 / Timer-XL、Kairos、Toto、IBM FlowState / TSPulse、TabPFN-TS 等纳入 frontier watchlist 审计。
...
| Frontier watchlist | Time-MoE、Sundial、Timer-S1 / Timer-XL、Kairos、Toto、IBM FlowState / TSPulse、TabPFN-TS | 先做结构化 audit；满足依赖、权重、接口、资源和 license 条件后再提升为实跑候选 |
...
项目已经从“单模型真实数据跑通”推进到“多开源模型真实执行评测入口”。下一步不应直接进入 B 阶段自研训练，也不应先做大规模公开数据集整理；更合适的是进入 C2.2，把当前六模型矩阵升级到 2025-2026 最新模型版本，并在真实 FU13 数据、统一任务口径和结构化失败 taxonomy 下产出一份能真正支撑 C3 或 B 阶段判断的报告。
