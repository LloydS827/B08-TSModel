...
当前阶段为 **model-core MVP sandbox + FU13 real data validation loop**。
...
- FU13 真实多 CSV 装配、连续炉 cycle 重构和场景诊断 CLI。
- 第一轮 forecasting 实验脚手架。
- 开源模型候选矩阵和 adapter optional dependency 检查。
- TTM 在 FU13 模拟数据上的本机 zero-shot forecasting 推理链路。
- baseline 和 TTM 在 FU13 真实数据窗口上的同口径验证。
- 专利、论文、benchmark、模型资产等知识成果规划。

尚未完成：

- 尚未完成 TimesFM、Chronos、Moirai 等更多模型的真实 zero-shot/frozen inference 对比。
- 尚未完成真实故障标签、维修记录、RUL 或维护建议闭环。
- 尚未完成围绕真实数据的算法效果提升和模型适配。
- 未训练自研设备时序基础模型。
...
4. 用真实数据上的同口径指标提升算法效果和异常候选信号质量。
5. 如果开源模型可用，优先 direct reuse 或轻量微调。
6. 只有当开源路线无法覆盖阶段、传感器、物理域和退化任务时，再考虑领域自训。
...
上面的 `90/16` 是已验证可运行的 TTM R2 子模型窗口配置。更长预测 horizon 需要改用匹配的 TTM 子模型窗口，例如更长 context，或后续实现 rolling 推理；不要把不兼容窗口下的 `runtime_failed` 误判为权重下载失败。

FU13 真实数据上的 TTM 对比命令：
...
模型权重、Hugging Face cache、真实现场导出数据、生成的 parquet 数据和临时实验报告只保存在本机。仓库已经忽略 `hf_cache/`、`model_cache/`、`models/`、`data/real/`、`data/simulated/*.parquet`、`data/processed/*.parquet` 和 `reports/*.md`，不要把这些文件上传到 GitHub。

## 真实数据对齐流程

真实数据入口分两类：通用导出使用 schema map 做格式验证；FU13 本轮现场导出使用 `configs/fu13_real_data_schema.yaml` 做多 CSV 装配、cycle 重构、诊断和 forecasting。
...
- 4,126,789 行 canonical observations。
- 8 个传感器、8 个工艺阶段。
- 428 个重构 cycle，其中 247 个完整 cycle。
- baseline 与 TTM 均已在 40 个 cross-stage 窗口上跑通。
- TTM 离线 cache 模式状态为 `available_and_ran`。
- 本阶段输出 scenario-level metrics；按单一业务场景过滤建窗属于下一轮算法验证。
- 当前 cycle 重构会把等待态归入相邻重构 cycle，后续再评估是否从训练/评测窗口中剔除。
...
docs/
  ...
  foundation-model-inference-plan.html # 真实基础模型推理实施计划
...
- FU13 真实数据可装配为 canonical observation 数据。
- baseline 和 TTM 可在真实数据窗口上生成同口径报告。
- 真实原始数据、生成 parquet、报告和模型 cache 不进入 Git。
- README 和 docs 能解释当前阶段、模型边界、运行命令和下一步路线。
- git 工作区除忽略产物和用户未纳入的文件外保持干净。

下一步建议：在已跑通的 FU13 真实数据闭环上做算法效果提升，先治理窗口选择、质量标记、异常候选信号和 baseline/TTM 对比口径，再继续做 TimesFM、Chronos、Moirai 等开源模型的 frozen inference 对比。
