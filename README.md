...
该 explicit local opt-in 只运行 TTM on FU13-like forecasting，并记录 dependency status、weight status、adapter status、runtime、input/output shape、actual network used 和 download allowed not verified。C-MAPSS RUL remains baseline-only：C3.3 不在 C-MAPSS RUL 上运行 open model adapter，C-MAPSS RUL 仍沿用 C3.2 的 RUL baseline evaluation。RUL metrics 和 forecasting metrics separated：RUL MAE / RMSE / NASA score 与 forecasting MAE / RMSE / residual ranking 不合并为单一排名。
